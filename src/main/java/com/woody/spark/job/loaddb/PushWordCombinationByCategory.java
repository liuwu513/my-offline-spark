 ...
 */
public class PushWordCombinationByCategory {

    /**
     * 最大推送数据量
     */
    private static final int maxPushSize = 5000;

    private static PinyinTool tool = new PinyinTool();

    public static void pushDataByLen(SparkContext sc, SparkSession sparkSession, Integer goodsCategory, Integer len) {
        Dataset<Row> goodsDF1 = sparkSession.read().schema(SchemaBuilder.nameCountsSchema()).format("json").
                json(String.format("/data/app/%d/combination%d.json", goodsCategory, len));
        if (goodsDF1.count() == 0) {
            return;
        }
        ...
        Dataset<RDDKeyByCounts> dataset = goodsDF1.selectExpr("name", "counts", String.format("%d as goodsCategory", goodsCategory),
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 45; i++) {
            for (int j = 2; j <= 4; j++) {
                pushDataByLen(sc, sparkSession, i, j);
            }
        }
        sparkSession.stop();
    }
